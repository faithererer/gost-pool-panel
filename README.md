...
```bash
docker pull ghcr.io/faithererer/gost-pool-panel:v0.3.8
```
...
- `IPv6 优先`：节点先走 IPv6-only 出口；如果目标有 AAAA 但本机 IPv6 出口异常导致连接失败，会自动重试 IPv4-only 出口。IPv4-only 目标也会走 IPv4。
