...
管理端 gost-pool-panel
  |-- Web UI / Admin API
  |-- 注册 token / install.sh / agent 下载
  |-- 代理池入口 GOST
  |-- JSON 状态存储
  |
  v
Linux 节点 gost-pool-agent
  |-- 心跳和任务轮询
  |-- 安装 / 配置 / 重启 GOST
  |-- 节点侧 HTTP/SOCKS5 代理
  |-- 流量统计上报
...
```bash
docker pull ghcr.io/faithererer/gost-pool-panel:v0.3.7
```
...
如果某些应用使用强制 IPv6 时返回 503，但 `curl https://api64.ipify.org` 正常，通常是目标服务 IPv4-only 或应用直接访问 IPv4 地址。优先改用 `IPv6 优先`。
